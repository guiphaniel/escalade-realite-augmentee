...
class Game:
    ...
    def startMultiMediaPipe(self):
        ...
        while self.continueGame:
            ...
            left = image[:, :width+50].copy()
            cv2.imshow("zitoune2", image)
            cv2.imshow("zitoune",left)
            resultsleft = rightPoseDetector.detectLandmarks(left)
            if resultsleft:
                self.transfoResults[0] = self.manager.wallCalibration.getTransformateLandmarks(resultsleft)
            right = image[:, width-50:].copy()
            resultsright = leftPoseDetector.detectLandmarks(right)
            if resultsright:
