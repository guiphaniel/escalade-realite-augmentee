...
class EditTextInternalFrame(AbstractInternalFrame, ActionListener, KeyboardListener):
    ...
    def __init__(self, coordinates, text, bgColor = (240, 240, 240), bgImage = None):
        AbstractInternalFrame.__init__(self, coordinates, bgColor, bgImage)
        ActionListener.__init__(self)
        KeyboardListener.__init__(self)
        self.text = text

        self.editText = EditText(0, 0, self.text)
        self.add(self.editText)
        self.backBt = Button(0, self.editText.rect.h + 20, text="RETOUR")
        self.add(self.backBt)
        self.validBt = Button(self.backBt.rect.w + 20, self.editText.rect.h + 20, text="VALIDER")
